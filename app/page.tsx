...
export default function Home() {
  ...
  function share(channel:"whatsapp"|"email"){const text=`Algoritmo criado no Portugol Fácil:\n\n${code}`;const url=channel==="whatsapp"?`https://wa.me/?text=${encodeURIComponent(text)}`:`mailto:?subject=${encodeURIComponent("Código Portugol: "+fileName())}&body=${encodeURIComponent(text)}`;window.open(url,"_blank","noopener,noreferrer");}
  return <main className="app-shell">
    ...
    <section className="workspace" id="editor">
      <div className="workspace-bar"><div className="file-tab"><span className="file-icon">◆</span> {fileName()} <span className="dot">●</span></div><div className="actions"><button className="ghost" onClick={()=>{setCode("");setOutput("");setStatus("pronto");}}>Novo</button><button className="ghost save" onClick={save} title="Baixar o código em formato .alg">↓ Salvar</button><button className="share whatsapp" onClick={()=>share("whatsapp")} title="Compartilhar pelo WhatsApp">WhatsApp</button><button className="share email" onClick={()=>share("email")} title="Enviar por e-mail">E-mail</button><button className="run" onClick={run}><span>▶</span> Executar <kbd>F9</kbd></button></div></div>
    ...
    <footer><span>Portugol Fácil</span><div className="footer-copy"><p>Um ambiente didático para dar os primeiros passos na programação.</p><small>Criado por <b>Anderson Marques Neto</b>, com o apoio da Inteligência Artificial.</small></div><a href="#editor">Voltar ao editor ↑</a></footer>
  </main>;
}
