...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace" id="editor">
      <div className="workspace-bar"><div className="file-tab"><span className="file-icon">◆</span> {fileName()} <span className="dot">●</span></div><div className="actions"><button className="ghost" onClick={()=>{setCode("");setOutput("");setStatus("pronto");}}>Novo</button><button className="ghost" onClick={()=>fileInputRef.current?.click()} title="Abrir arquivo .alg ou .txt">↑ Abrir</button><input ref={fileInputRef} className="file-input" type="file" accept=".alg,.txt,text/plain" onChange={openFile}/><button className="ghost save" onClick={save} title="Baixar o código em formato .alg">↓ Salvar</button><button className="ghost pdf" onClick={printPdf} title="Imprimir ou salvar o código em PDF">PDF</button><button className="share whatsapp" onClick={()=>share("whatsapp")} title="Compartilhar arquivo pelo WhatsApp">WhatsApp</button><button className="share email" onClick={()=>share("email")} title="Compartilhar arquivo por e-mail">E-mail</button><button className="run" onClick={run}><span>▶</span> Executar <kbd>F9</kbd></button></div></div>
    ...
    <footer><span>Portugol Fácil <i className="version">v{APP_VERSION}</i></span><div className="footer-copy"><p>Um ambiente didático para dar os primeiros passos na programação.</p><small>Criado por <b>Anderson Marques Neto</b>, com o apoio da Inteligência Artificial.</small></div><a href="#editor">Voltar ao editor ↑</a></footer>
  </main>;
}
